...
const { startStatic, vendorRoute } = require('./_harness.cjs');
...
(async () => {
  ...
  const info = await page.evaluate(() => {
    ...
    // 待办行 = todo-center.js 渲染的 <article class="todo-card" data-task="…">
    const rows = deep('.todo-card, [data-task]').filter(e => e.matches && e.matches('.todo-card, [data-task]'));
