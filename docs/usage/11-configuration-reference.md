...
> **无环境变量配置**（2026-09-02 终态）：订阅以库中 `cmx_flow_webhook_subscription` 表为
> 唯一真源，管理页面 / REST 维护（见 [09 §9.14](09-operations-and-admin.md)）。历史上的
> `FLOW_WEBHOOK_MODE` / `FLOW_WEBHOOK_MAX_RETRIES`（legacy 链路）与
> `FLOW_WEBHOOK_TARGETS` / `FLOW_WEBHOOK_SIGNING_KEY`（首启种子导入）均已删除，配置残留
> 无害（不再被读取）。新环境接入见 [08 §8.6](08-external-integration.md) 的订阅创建样例；
> 每订阅 `channel_config.secret` 须与接收端共享密钥一致（如 mdm 的
> `[mdm.flow].webhook_secret`），重试上限在订阅行 `retry_max`（默认 10 含首发）。
